...
class Agent(NAE):
    ...
    def __init__(self):
        ...
        self.init_alert_description({VSX: NORMAL})
    ...
    def create_monitors(self):
        ...
        for rule in RESOURCE_URI_MAP:
            uri = URI_PREFIX_MONITOR + RESOURCE_URI_MAP[rule]
            monitors[rule] = {
    ...
    def update_alert_level(self):
        ...
        if (neighbors_ratio == AlertLevel.CRITICAL and self.get_alert_level() != AlertLevel.CRITICAL):
            description = "The ratio of neighbors count on the switch to the neighbors count on the VSX-peer is outside the allowed deviation"
            self.set_alert_description_for_key(VSX, description)
            self.set_alert_level(AlertLevel.CRITICAL)
        elif (mac_addresses_ratio == AlertLevel.CRITICAL and self.get_alert_level() != AlertLevel.CRITICAL):
            description = "The ratio of mac addresses count on the switch to the mac addresses count on the VSX-peer is outside the allowed deviation"
            self.set_alert_description_for_key(VSX, description)
            self.set_alert_level(AlertLevel.CRITICAL)
            ...
            description = "The ratio of routes count on the switch to the routes count on the VSX-peer is outside the allowed deviation"
            self.set_alert_description_for_key(VSX, description)
            self.set_alert_level(AlertLevel.MINOR)
        elif (neighbors_ratio == 'Normal' and mac_addresses_ratio == 'Normal'
              and routes_ratio == 'Normal') and (self.get_alert_level() is not None):
            self.clear_alert_description_for_key(VSX)
    ...
    def get_count_values(self, resource):
        ...
        """
        count_values = {}
        rest_uri = URI_PREFIX_GET + RESOURCE_URI_MAP[resource]
        vsx_rest_uri = VSX_PEER + rest_uri
        try:
            r = self.get_rest_request_json(HTTP_ADDRESS + rest_uri)
            count = r["count"]
        ...
        try:
            r = self.get_rest_request_json(HTTP_ADDRESS + vsx_rest_uri)
            count = r["count"]
