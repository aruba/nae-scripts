...
class Policy(NAE):
    ...
    def __init__(self):
        ...
        self.r1.clear_action(self.action_lc_down_clear)
    ...
    def action_lc_down_clear(self, event):
        self.logger.debug("======= LC Down clear ============")
        lc_slot = self.get_lc_slot(event['labels'])
        lc_number = str(self.get_lc_number(lc_slot))
        ActionSyslog('Line Card [' + lc_slot + '] down status cleared')
        lc_down_list = self.variables['lc_down_list']
        self.logger.debug('lc_down_list before: [' + lc_down_list + ']')
        if lc_number in lc_down_list:
            lc_down_list = lc_down_list.replace(lc_number, '')
            self.variables['lc_down_list'] = lc_down_list
            if not lc_down_list:
